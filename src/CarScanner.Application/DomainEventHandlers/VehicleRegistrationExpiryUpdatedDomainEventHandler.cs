...
public sealed class VehicleRegistrationExpiryUpdatedDomainEventHandler(
    ITenantProvider tenantProvider,
    IMaintenanceReminderRepository reminderRepository,
    IUnitOfWork unitOfWork,
    ILogger<VehicleRegistrationExpiryUpdatedDomainEventHandler> logger)
    : IDomainEventHandler<VehicleRegistrationExpiryUpdatedDomainEvent>
{
    public async Task Handle(
        VehicleRegistrationExpiryUpdatedDomainEvent notification,
        CancellationToken cancellationToken)
    {
        ...
        var existing = await reminderRepository.GetByVehicleAndTypeAsync(
            notification.VehicleId, ReminderType.RegistrationExpiry, cancellationToken);

        if (notification.NewExpiry is { } newExpiry)
        {
            var description = $"Registracija ističe {newExpiry:dd.MM.yyyy}";

            if (existing is null)
            {
                var result = MaintenanceReminder.Create(
                    tenantId, notification.VehicleId, ReminderType.RegistrationExpiry,
                    newExpiry, dueMileage: null, description);

                if (result.IsSuccess)
                    reminderRepository.Add(result.Value);
                else
                    logger.LogWarning("Failed to create registration reminder: {Error}", result.Error.Code);
            }
            else
            {
